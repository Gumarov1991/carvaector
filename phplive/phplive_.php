<?php
	...
	if ( ( !isset( $CONF['cookie'] ) || ( isset( $CONF['cookie'] ) && ( $CONF['cookie'] == "on" ) ) ) && !isset( $_COOKIE["phplive_vid"] ) ) { setcookie( "phplive_vid", Util_Format_RandomString(10)+time(), time()+(60*60*24*180), "/" ) ; }
	...
	if ( $deptid && $vname )
	{
		...
		if ( $lang == "brazilian" ) { foreach ( $LANG as $lang_ref => $lang_text ) { $LANG[$lang_ref] = utf8_encode( $lang_text ) ; } }
...
<!--
	...
	$(document).ready(function()
	{
		...
		var profile_pic_class_found = 0 ;
		var rules = document.styleSheets[0].rules || document.styleSheets[0].cssRules ;
		for ( var the_index in rules ) {
            if ( rules[the_index].selectorText == ".profile_pic_img" ) {
				profile_pic_class_found = 1 ; return true ;
			}
        } if ( !profile_pic_class_found ) { $('#chat_profile_pic').addClass('info_box') ; }
	});
	...
	function init_resize_chat_body( chat_body_height )
	{
		...
		// when browser window is placed in background a while
		if ( typeof( si_chat_body_resize ) != "undefined" ) { clearInterval( si_chat_body_resize ) ; si_chat_body_resize = undeefined ; }
		si_chat_body_resize = setInterval(function(){
			var chat_body_height_temp = $('#chat_body').height() ;
			if ( chat_body_height_temp != chat_body_height ) { $('#chat_body').css({'height': chat_body_height}) ; init_scrolling() ; }
		}, 200) ;
	}
