...
<!--
	...
	function confirm_emo_onoff( thedeptid, thevalue )
	{
		...
		if ( confirm( "Switch the emoticons "+thevalue+"?" ) )
		{
		...
		{
			var new_value = ( thevalue == "on" ) ? "off" : "on" ;
			if ( thedeptid )
			{
				if ( dept_emos[thedeptid] != thevalue )
				{
					dept_emos[thedeptid] = new_value ;
					$('#dept_emo_'+thedeptid+"_"+new_value).prop('checked', true) ;
				}
				else
					dept_emos[thedeptid] = thevalue ;
			}
...
<?php include_once( "../../setup/inc_header.php" ) ?>
		...
		<div style="margin-top: 25px;">
			...
			<div style="margin-top: 15px;" class="info_info">
				<div class="info_good" style="float: left; width: 260px; padding: 3px;"><input type="radio" name="dept_emo_0" id="dept_emo_0_on" value="on" onClick="confirm_emo_onoff(0, this.value)"> Enable Emoticons for ALL Departments</div>
				<div class="info_error" style="float: left; margin-left: 10px; width: 260px; padding: 3px;"><input type="radio" name="dept_emo_0" id="dept_emo_0_off" value="off" onClick="confirm_emo_onoff(0, this.value)"> Disable Emoticons for ALL Departments</div>
				...
				<?php
					for ( $c = 0; $c < count( $departments ); ++$c )
					{
						...
						if ( $department["name"] != "Archive" )
						{
							...
							$div_onoff = "<div style=\"margin-top: 15px;\"><div class=\"info_good\" style=\"float: left; width: 60px; padding: 3px; text-shadow: none;\"><input type=\"radio\" name=\"dept_emo_$department[deptID]\" id=\"dept_emo_$department[deptID]_on\" value=\"on\" onClick=\"confirm_emo_onoff($department[deptID], this.value)\" $checked_on> On</div><div class=\"info_error\" style=\"float: left; margin-left: 10px; width: 60px; padding: 3px; text-shadow: none;\"><input type=\"radio\" name=\"dept_emo_$department[deptID]\" id=\"dept_emo_$department[deptID]_off\" value=\"off\" onClick=\"confirm_emo_onoff($department[deptID], this.value)\" $checked_off> Off</div><div style=\"clear: both;\"></div></div>" ;
