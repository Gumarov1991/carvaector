<?php
	...
	else if ( $action == "submit" )
	{
		...
		else if ( $menu == "sa" )
		{
			...
			if ( isset( $admininfo["adminID"] ) && ( $password == md5($admininfo["password"]) ) )
			{
				$ses = md5( $now.$ip ) ;
		...
		else
		{
			...
			if ( isset( $opinfo["opID"] ) && ( $password == md5($opinfo["password"]) ) )
			{
	...
	if ( !$login && $auto_login_token && $auto_login_token_ses && ( $menu != "sa" ) )
	{
		...
		{
			$md5_password = md5( $opinfo_["password"] ) ;
			$remember = 1 ;
		} else { setcookie( "phplive_auto_login_token", FALSE, -1, "/" ) ; }
	}
...
<!--
	...
	function toggle_menu( themenu )
	{
		...
		if ( themenu == "sa" )
		{
			...
			$('#div_info_operator').hide() ; $('#div_info_setup').show() ;
			$('#phplive_login').val( "" ) ;
			if ( screen_ == "same" ) { }
			$('#copyright').show() ;
	...
	function do_login()
	{
		...
		{
			var md5_password = phplive_md5( phplive_md5( $('#phplive_password_temp').val() ) ) ;
			$('#phplive_password').val( md5_password ) ;
			$('#theform').submit() ;
		}
	}
...
<div id="body" style="padding-bottom: 20px;">
	<div style="width: 100%; padding-top: 30px;">
		...
		<div class="info_neutral" style="width: 280px; height: 340px; margin: 0 auto; margin-top: 15px; padding: 10px; text-shadow: 1px 1px #FFFFFF;">

			<div id="div_info_operator" class="info_info" style="display: none; text-align: center;"><img src="pics/icons/agent.png" width="16" height="16" border="0" alt=""> Chat Operator Login</div>
			<div id="div_info_setup" class="info_info" style="display: none; text-align: center;"><img src="pics/icons/settings.png" width="16" height="16" border="0" alt=""> Setup Admin Login</div>
			...
			<tr>
				<td><span id="div_txt_login">Login</span></td>
			...
			<tr>
				...
				<td colspan=3>
					...
					<div id="div_btn_forgot" style="display: none; margin-top: 10px;">
						<div class="info_error" style="display: none; text-shadow: none;" id="div_forgot_error_sa">Please contact the Setup Admin to reset your login credentials.</div>
						<div class="info_error" style="display: none; margin-top: 5px; text-shadow: none;" id="div_forgot_error_op">Update the Setup Admin credentials directly at the database.</div>
						...
						<div style="margin-top: 15px;">
							...
							<div style="margin-top: 35px;">&bull; <a href="JavaScript:void(0)" onClick="toggle_forgot(0)">back to login</a></div>
						</div>
					</div>
				</td>
			</tr>
		...
		<div style="padding: 5px;">
			<div style="width: 280px; margin: 0 auto; font-size: 10px; text-shadow: 1px 1px #FFFFFF;">
