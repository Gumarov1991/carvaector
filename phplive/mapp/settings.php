<?php
	...
	if ( ( $action == "pre_register" ) && $url )
	{
		if ( function_exists( "curl_init" ) )
		{
			...
			if ( $response == 0 ) { $error = "X-Frame-Options detected.  For more information please visit the <a href='http://www.phplivesupport.com/r.php?r=xframe' target='_blank' style='color: #FFFFFF;'>X-Frame-Options documentation</a>." ; $json_data = "json_data = { \"status\": 0, \"error\": \"$error\" };" ; }
			else if ( $response == 1 ) { $json_data = "json_data = { \"status\": 1 };" ; }
			else { $json_data = "json_data = { \"status\": 0, \"error\": \"Invalid URL.\" };" ; }
		}
		else
		{
			$json_data = "json_data = { \"status\": 0, \"error\": \"Server PHP does not support <a href='http://php.net/manual/en/book.curl.php' target='_blank' style='color: #FFFFFF;'>cURL</a>.  Contact your server admin to enable the PHP cURL support to utilize the Mobile App feature.\" };" ;
		}
...
<?php include_once( "../setup/inc_header.php" ) ?>
			...
			<div class="op_submenu_wrapper">
				...
				<div class="op_submenu" onClick="location.href='../setup/db.php?ses=<?php echo $ses ?>'" id="menu_cookie">System</div>
			...
			<div id="div_register" style="display: none; margin-top: 25px;">
				...
				<div style="margin-top: 25px;" class="info_info">
					...
					<div style="margin-top: 5px; font-size: 32px; font-weight: bold; color: #3FABC1; text-shadow: 1px 1px #FFFFFF;"><span id="phplive_url"></span></div>
				</div>
