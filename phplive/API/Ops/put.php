<?php
					...
					$email )
	{
		...
		LIST( $opid, $status, $mapper, $rate, $sms, $op2op, $traffic, $viewip, $nchats, $maxc, $password, $name, $email ) = database_mysql_quote( $dbh, $opid, $status, $mapper, $rate, $sms, $op2op, $traffic, $viewip, $nchats, $maxc, $password, $name, $email ) ;

		if ( isset( $operator["opID"] ) )
			$query = "UPDATE p_operators SET mapper = $mapper, rate = $rate, op2op = $op2op, traffic = $traffic, viewip = $viewip, nchats = $nchats, maxc = $maxc, sms = $sms, login = '$login', password = '$password', name = '$name', email = '$email' WHERE opID = $opid" ;
		else
			$query = "INSERT INTO p_operators VALUES ( $opid, 0, 0, 0, $mapper, 0, 0, $rate, $op2op, $traffic, $viewip, $nchats, $maxc, 0, '', '', 0, $sms, '', '$login', '$password', '$name', '$email', 0, 'default' )" ;
					...
					$opid )
	{
		...
		$query = "INSERT INTO p_op_vars VALUES( $opid, 0, 1, 0, 0, 1, 1, 0, 1, 0, 0 )" ;
		database_mysql_query( $dbh, $query ) ;
