<?php
	...
	FUNCTION Ops_update_itr_IdleOps( &$dbh )
	{
		...
		$query = "SELECT * FROM p_operators WHERE ( ( lastactive < $idle AND mapp = 0 ) OR ( lastactive < $idle_mapp AND mapp = 1 ) ) AND status = 1" ;
		...
		if ( $dbh[ 'ok' ] )
		{
			if ( database_mysql_nresults( $dbh ) )
			{
				...
				for( $c = 0; $c < count( $operators ); ++$c )
				{
					...
					$query = "INSERT INTO p_opstatus_log VALUES( $now, $operator[opID], 0, 0 )" ;
					database_mysql_query( $dbh, $query ) ;
					...
					if ( $operator["mapp"] )
					{
						...
						$query = "UPDATE p_operators SET ses = 'mapp_idle' WHERE opID = $operator[opID]" ;
						database_mysql_query( $dbh, $query ) ;
