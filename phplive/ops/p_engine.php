...
<!--
	...
	function requesting()
	{
		...
		data: "m="+parent.mapp+"&a=rq&ps="+parent.prev_status+"&t="+parent.traffic+"&cr="+c_requesting+"&"+q_ces+"&"+unique+"&",
		success: function(data){
			if ( typeof( st_network ) != "undefined" ) { clearTimeout( st_network ) ; st_network = undeefined ; }
			try {
				eval(data) ;
	...
	function chatting()
	{
		...
		if ( start )
		{
			...
			data: "rq="+requestid+"&t="+t_vses+"&o="+isop+"&o_="+isop_+"&o__="+isop__+"&c="+thisces+"&ch="+c_chatting+"&"+q_ces+q_chattings+q_isop_+q_isop__+"&m="+mobile+"&mp="+mapp+"&"+unique+"&",
			success: function(data){
				try {
					eval(data) ;
				} catch(err) {
