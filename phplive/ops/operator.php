...
<!--
	...
	function prep_init()
	{
		if ( !mapp ) { $("#div_input_text").dblclick(function() { toggle_input_text() ; }); }
		else
		{
	...
	function activate_chat( theces )
	{
		if ( mapp )
		{
			...
			// mapp has switchboard position so best to close_extra
			close_extra( extra ) ;
		}
	...
	{
		if ( typeof( document.getElementById('iframe_ext_'+thediv).contentWindow.display_error ) != "undefined" ) { document.getElementById('iframe_ext_'+thediv).contentWindow.display_error( theerror, theurl ) ; }
	...
	function close_extra_doit( thediv )
	{
		if ( typeof( thediv ) != "number" )
		{
			if ( ( thediv == "op2op" ) || ( thediv == "traffic" ) || ( thediv == "settings" ) || ( thediv == "mapp_themes" ) || ( thediv == "mapp_prefs" ) || ( thediv == "mapp_sounds" ) || ( thediv == "mapp_operators" ) || ( thediv == "mapp_power" ) || ( thediv == "mapp_traffic" ) ) { iframe_blank( thediv ) ; }
		}
		extra = undeefined ;
		reset_footer() ;
	}
	...
	function hide_extra( thediv )
	{
		...
		for ( var c = 0; c < obj_length; ++c )
		{
			if ( divs[c] != thediv )
			{
				...
				if ( ( thisdiv == "op2op" ) || ( thisdiv == "traffic" ) || ( thisdiv == "settings" ) || ( thisdiv == "mapp_themes" ) || ( thisdiv == "mapp_prefs" ) || ( thisdiv == "mapp_sounds" ) || ( thisdiv == "mapp_operators" ) || ( thisdiv == "mapp_power" ) || ( thisdiv == "mapp_traffic" ) ) { iframe_blank( thisdiv ) ; }
		...
		if ( prev_status != thestatus )
		{
			...
			else
			{
				...
				else if ( ( thestatus == 3 ) || ( thestatus == 4 ) )
				{
					...
					location.href = base_url+"/logout.php?action=logout&ao="+ao+"&rd="+rd+"&dup="+dup+"&mi="+mi+"&auto=<?php echo $auto ?>&wp="+wp+"&pop=<?php echo $pop ?>&mapp="+mapp+"&"+unique+"&" ;
				}
				else
				{
					$('input:radio[name=status]')[prev_status].checked = true ;
	...
	function reload_console( theflag )
	{
		...
		window.onbeforeunload = null ;
		location.href = base_url_full+"/ops/operator.php?reload=1&ses=<?php echo $ses ?>&wp=<?php echo $wp ?>&auto=<?php echo $auto ?>&mapp="+mapp+"&nalert="+theflag+"&"+unique+"&" ;
	}
		...
		<div id="chat_info_container">
			...
			<div id="chat_info_wrapper_info">
				<div id="chat_info_menu_list">
					...
					<div id="info_menu_footprints" class="chat_info_menu" onClick="toggle_info('footprints',1)" style="display: none;">Footprints</div>
					<?php endif ; ?>
				...
				<div id="chat_info_body" style="overflow: auto;">
					<div id="info_info" style="display: none; padding: 10px; text-align: justify;">
						<div id="info_info_body" style="">
							...
							<tr><td class="chat_info_td_h">Department</td><td width="100%" class="chat_info_td"> <span id="req_dept"></span></td></tr>
...
<div id="chat_btn" style="position: absolute; padding-right: 10px; z-Index: 10"><button id="input_btn" type="button" OnClick="add_text_prepare(1)" disabled="disabled">Submit</button><div style="margin-top: 5px; font-size: 10px;" id="chat_text_login">logged in as:<br><?php echo $opinfo["login"] ?></div></div>
...
<div id="chat_footer" style="position: absolute; width: 100%; bottom: -2px; padding-top: 2px; height: 27px; z-Index: 102;">
