var chat_http_error ; var st_http ;
...
function add_text_prepare()
{
	...
	if ( isop && shortcut_enabled ) {
		if ( thetext.match( /^\// ) ) { process_shortcuts( thetext ) ; return true ; }
	}
...
function init_divs( theresize )
{
	...
	if ( isop && ( typeof( profile_pic_enabled ) != "undefined" ) && profile_pic_enabled )
	{
		input_text_width -= 65 ;
...
function update_ces( thejson_data )
{
	...
	if ( ( typeof( chats[thisces] ) != "undefined" ) )
	{
		...
		if ( thejson_data["text"].indexOf( "<disconnected>" ) != -1 )
		{
			...
			if ( isop ) { clearInterval( chats[thisces]["timer_si"] ) ; chats[thisces]["timer_si"] = undeefined ; } else { document.getElementById('iframe_chat_engine').contentWindow.stopit(0) ; }
		}
...
function disconnect( theunload, theclick, theces, thevclick )
{
	...
	if ( ( ( typeof( theces ) != "undefined" ) && ( typeof( chats[theces] ) != "undefined" ) ) || widget )
	{
		...
		if ( !chats[theces]["disconnect_click"] )
		{
			...
			data: "action=disconnect&isop="+isop+"&isop_="+isop_+"&isop__="+isop__+"&ces="+theces+"&ip="+chats[theces]["ip"]+"&widget="+widget+"&t_vses="+chats[theces]["t_ses"]+"&idle="+chats[theces]["idle_counter"]+"&vclick="+thevclick+"&unload="+theunload+"&unique="+unique,
			success: function(data){
				try {
					eval(data) ;
				} catch(err) {
