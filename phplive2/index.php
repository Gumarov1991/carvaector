<?php
			...
			if ( isset( $opinfo["opID"] ) && ( $password == md5($opinfo["password"].$token) ) )
			{
				...
				if ( $auto_login_token_ses )
				{
					if ( $opinfo["ses"] != $auto_login_token_ses )
					{
						...
						database_mysql_close( $dbh ) ;
						HEADER( "location: logout.php?action=logout&dup=1&wp=$wp&auto=$auto&menu=operator&wpress=$wpress&$now" ) ;
					}
					$remember = 1 ;
				}
	...
	if ( !$login && $auto_login_token && $auto_login_token_ses && ( $menu != "sa" ) )
	{
		...
		if ( isset( $opinfo_["opID"] ) )
		{
			...
			HEADER( "location: index.php?action=submit&phplive_password=$md5_password&phplive_login=$opinfo_[login]&remember=1&auto=$auto&wp=$wp&menu=operator&wpress=$wpress&ses=$auto_login_token_ses&$now" ) ;
		} else { setcookie( "phplive_auto_login_token", FALSE ) ; }
	}
	...
	if ( !Ops_get_itr_AnyOpsOnline( $dbh, 0 ) )
	{
		...
		if ( $total_dir_files )
		{
			...
			{
				if ( $dir_files[$c] ) { unlink( $dir_files[$c] ) ; }
			}
		}
	}
...
<!--
		...
		<?php
			if ( ( $action == "submit" ) && ( $menu == "operator" ) && !$error )
			{
				...
				else
				{
					...
					if ( $wp || $auto )
						print "setTimeout( function(){ location.href='ops/operator.php?ses=$ses&auto=$auto&wp=$wp' ; }, 4000 ) ;" ;
					else
						print "setTimeout( function(){ location.href='ops/?ses=$ses&auto=$auto&wp=$wp' ; }, 4000 ) ;" ;
				}
			}
	...
	function check_protocol()
	{
		...
		{
			location.href = base_url+"/?<?php echo $query ?>" ;
		}
