...
<!--
	...
	function requesting()
	{
		...
		data: "a=rq&ps="+parent.prev_status+"&t="+parent.traffic+"&cr="+c_requesting+"&"+q_ces+"&"+unique,
		success: function(data){
			if ( typeof( st_network ) != "undefined" ) { clearTimeout( st_network ) ; st_network = undeefined ; }
			try {
				eval(data) ;
			...
			if ( !stopped || ( stopped && reconnect ) )
			{
				...
				else if ( json_data.status )
				{
					...
					{
						if ( json_data.requests[c]["op2op"] || ( typeof( parent.op_depts_hash[json_data.requests[c]["did"]] ) != "undefined" ) )
						{
	...
	function chatting()
	{
		var json_data = new Object ;
		var start = 0 ;
		var q_ces = "", q_chattings = "", q_isop_ = "", q_isop__ = "" ;
		...
		if ( start )
		{
			...
			data: "rq="+requestid+"&t="+t_vses+"&o="+isop+"&o_="+isop_+"&o__="+isop__+"&c="+thisces+"&ch="+c_chatting+"&"+q_ces+q_chattings+q_isop_+q_isop__+"&m="+mobile+"&"+unique,
			success: function(data){
				try {
					eval(data) ;
				} catch(err) {
