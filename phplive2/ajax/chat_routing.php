<?php
	...
	*/
	...
	if ( $action == "routing" )
	{
		...
		$c_routing = Util_Format_Sanatize( Util_Format_GetVar( "cr" ), "ln" ) ;
		$rtype = Util_Format_Sanatize( Util_Format_GetVar( "r" ), "ln" ) ;
		$rtime = Util_Format_Sanatize( Util_Format_GetVar( "rt" ), "ln" ) ;
		$rloop = Util_Format_Sanatize( Util_Format_GetVar( "rl" ), "ln" ) ;
		$loop = Util_Format_Sanatize( Util_Format_GetVar( "l" ), "ln" ) ;
		...
		else
		{
			if ( $requestinfo["status"] && ( $requestinfo["opID"] != 1111111111 ) )
			{
				...
				$json_data = "json_data = { \"status\": 1, \"status_request\": $requestinfo[status], \"requestid\": $requestinfo[requestID], \"initiated\": $requestinfo[initiated], \"name\": \"$opinfo[name]\", \"rate\": $opinfo[rate], \"deptid\": $deptid, \"opid\": $opinfo[opID], \"email\": \"$opinfo[email]\", \"profile\": \"$profile_src\" };" ;
			}
			else
			{
